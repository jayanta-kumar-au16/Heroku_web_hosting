...
const playerOne = (a) => {
  ...
  function check() {
    for (let i = 0; i < movement.length; i++) {
      let checker = () =>
        movement[i].answer.every((v) => playerOneState.includes(v));
      if (checker()) {
        playerOneWin = true;
...
const playerTwo = (a) => {
  ...
  a.currentTarget.classList.add("green");
  counter++;
  playerTwoState.push(Number(box.dataset.number));
  function check() {
    for (let i = 0; i < movement.length; i++) {
      let checker = () =>
        movement[i].answer.every((v) => playerTwoState.includes(v));
      if (checker()) {
        playerTwoWin = true;
        results.classList.add("display");
        text.innerText = "green player wins!";
        results.classList.add("greenWins");
      }
    }
  }
  check();
...
reset.addEventListener("click", () => {
  ...
  results.classList.remove("greenWins");
  blocks.forEach((e) => {
